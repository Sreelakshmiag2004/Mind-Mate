...
void main() {
  group('MediaAssetModel.fromJson — metadata', () {
    ...
    test('a null title parses as null (never renamed yet — not an error state)', () {
    ...
    test('a set title parses correctly', () {
      final asset = MediaAssetModel.fromJson(_listItemJson(title: 'Beach trip'));

      expect(asset.title, 'Beach trip');
    });

    test('a null duration_seconds parses as null (image/video with no duration)', () {
      final asset = MediaAssetModel.fromJson(_listItemJson(durationSeconds: null));

      expect(asset.durationSeconds, isNull);
    });

    test('a set duration_seconds (voice) parses correctly', () {
      final asset = MediaAssetModel.fromJson(_listItemJson(mediaType: 'voice', durationSeconds: 42));

      expect(asset.mediaType, 'voice');
      expect(asset.durationSeconds, 42);
    });

    test('created_at parses into a real DateTime, consistent with the rest of the project', () {
      final asset = MediaAssetModel.fromJson(_listItemJson(createdAt: '2025-01-06T09:15:00+00:00'));
  ...
  group('MediaAssetModel.fromJson — list vs. detail shape', () {
    ...
    test('a detail (MediaAssetDetail) response carries download_url and its expiry', () {
      final json = {
        ..._listItemJson(),
        'download_url': 'https://storage.example/bucket/image/u1/abc.png?X-Amz-Signature=...',
        'download_url_expires_in_seconds': 900,
      };

      final asset = MediaAssetModel.fromJson(json);

      expect(asset.downloadUrl, 'https://storage.example/bucket/image/u1/abc.png?X-Amz-Signature=...');
      expect(asset.downloadUrlExpiresInSeconds, 900);
    });
  });

  group('MediaAssetModel.fromJson — never carries fields it must not', () {
    test('does not decode user_id, object_key, or updated_at even if present in the JSON', () {
      final json = {
        ..._listItemJson(),
  ...
  group('MediaAssetModel.toJson', () {
    test('round-trips every decoded field, list-shaped (no download_url)', () {
      final asset = MediaAssetModel.fromJson(_listItemJson(title: 'Renamed', durationSeconds: 10));
  ...
  group('MediaAssetPage.fromJson', () {
    test('parses items/total/limit/offset from a Page[MediaAssetRead] response', () {
      final json = {
        'items': [_listItemJson(id: 'a'), _listItemJson(id: 'b', mediaType: 'voice')],
        'total': 2,
        'limit': 30,
        'offset': 0,
      };
    ...
    test('parses an empty page correctly', () {
      final page = MediaAssetPage.fromJson({'items': [], 'total': 0, 'limit': 30, 'offset': 0});
